...
def C3D(input, num, dimensions, dropout=False, regularizer=True):

    if regularizer: regularizer = tf.contrib.layers.l2_regularizer(0.0005) # 0.005 正则化L2
    ...
    with tf.variable_scope('C3D', reuse=tf.AUTO_REUSE):
        ...
        reshaped = tf.reshape(maxpool_5, [num, nodes])  # pool_shape[0] is N, batch_size


        #dense1 = tf.reshape(maxpool_5, [10, 8192].get_shape().as_list()[0])
        #dense1 = tf.reshape(pool5, [batch_size, _weights['wd1'].get_shape().as_list()[0]])
        with tf.variable_scope('fc6'):
            fc6_weight = tf.get_variable('weight', [nodes, 4096],
                                         initializer=tf.random_normal_initializer(stddev=0.005))
            if regularizer:
                tf.add_to_collection("weight_decay_loss", regularizer(fc6_weight))
            fc6_bias = tf.get_variable('bias', [4096], initializer=tf.constant_initializer(1.0))
            fc6 = tf.nn.relu(tf.matmul(reshaped, fc6_weight) + fc6_bias)
            if dropout:
                fc6 = tf.nn.dropout(fc6, 0.5)

        with tf.variable_scope('fc7'):
            fc7_weight = tf.get_variable('weight', [4096,4096], initializer=tf.random_normal_initializer(stddev=0.005))
            if regularizer:
                tf.add_to_collection("weight_decay_loss", regularizer(fc7_weight))
            fc7_bias = tf.get_variable('bias', [4096], initializer=tf.constant_initializer(1.0))
            fc7 = tf.nn.relu(tf.matmul(fc6, fc7_weight) + fc7_bias)
            if dropout:
                fc7 = tf.nn.dropout(fc7, 0.5)

        with tf.variable_scope('fc8'):  # fc8
            out_weight = tf.get_variable('weight', [4096, dimensions],
